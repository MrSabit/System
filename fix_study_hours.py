#!/usr/bin/env python
# Script to fix the get_study_hours function in routes.py

def fix_study_hours_function(filename):
    with open(filename, 'r', encoding='utf-8') as file:
        content = file.read()
    
    # First, locate the get_study_hours function
    start_marker = "@bp.route('/api/study_hours', methods=['POST'])"
    end_marker = "@bp.route('/api/add_skill', methods=['POST'])"
    
    if start_marker not in content or end_marker not in content:
        print("Error: Could not find the get_study_hours function")
        return False
    ...
    # Create a new properly formatted function
    new_function = """@bp.route('/api/study_hours', methods=['POST'])
@login_required
def get_study_hours():
...
"""
    ...
    print(f"Fixed get_study_hours function in {filename}")
    return True

if __name__ == "__main__":
    fix_study_hours_function("app/routes.py") 