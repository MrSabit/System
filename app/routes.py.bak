...
def record_study():
    try:
        ...
        # Since subject is now independent, always use General Study
        # Find existing general study subject or create one
        ...
        if g.user.xp >= g.user.xp_to_next_level:
            ...
            g.user.xp_to_next_level = 100 + 10 * g.user.level  # New linear formula
            level_up = True
            flash(f'Study session recorded! You earned {actual_xp} XP and leveled up to level {g.user.level}!')
        else:
...
def toggle_topic(topic_id):
    ...
    topic.is_completed = not was_completed
    ...
    if not was_completed and topic.is_completed:
        ...
        if not request.is_json:
            flash(message)
    elif was_completed and not topic.is_completed:
        message = f'Topic "{topic.name}" marked as not completed'
        if not request.is_json:
            flash(message)
...
def get_study_data():
    ...
    return jsonify({
        ...
        chart_labels = [datetime.strptime(date, '%Y-%m-%d').strftime('%d') for date in dates]
        chart_data = [hours_by_date[date] for date in dates]
        ...
        for i in range(11, -1, -1):
            ...
            if current_month.month == 12:
                last_day_of_month = current_month.replace(year=current_month.year + 1, month=1, day=1) - timedelta(days=1)
            else:
                last_day_of_month = current_month.replace(month=current_month.month + 1, day=1) - timedelta(days=1)
            
            # Query study sessions for this month
            monthly_sessions = StudySession.query.filter_by(user_id=g.user.id).filter(
                StudySession.date >= first_day_of_month,
                StudySession.date <= last_day_of_month
            ).all()
            
            # Sum hours
            total_hours = sum(session.hours for session in monthly_sessions)
        ...
        total_hours = sum(session.hours for session in all_sessions)
        total_sessions = len(all_sessions)
        avg_hours = round(total_hours / total_sessions, 1) if total_sessions > 0 else 0
        
        # Calculate streaks
        date_set = set(session.date for session in all_sessions)
        today = datetime.now().date()
        ...
        for i in range(len(date_list)):
            if i == 0 or date_list[i] - date_list[i-1] == timedelta(days=1):
                temp_streak += 1
            else:
                temp_streak = 1
