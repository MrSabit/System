...
from app.models import User, Objective, ProgressRecord, QuestCategory, QuestSettings, Mission, PenaltyPeriod, QuestCompletionRecord, Subject, Topic, StudySession, Subtopic, CustomSkill, JournalEntry, db
from app import db
...
def dashboard():
    ...
    # Get all custom skills for the user
    custom_skills = CustomSkill.query.filter_by(user_id=g.user.id).all()
    
    # Convert custom skills to the same format as built-in skills
    custom_skills_list = [
        {
            "name": skill.name,
            "category": skill.category,
            "icon": skill.icon,  # Store raw emoji
            "value": skill.level,
            "is_custom": True
        }
    ...
    # Define all built-in skills with their categories and icons
    built_in_skills = [
        ...
        {"name": "Stamina", "category": "Physical", "icon": "🔋", "value": g.user.stamina},
        ...
        {"name": "Resilience", "category": "Mental", "icon": "🛡️", "value": g.user.resilience},
        {"name": "Speaking", "category": "Communication", "icon": "🗣️", "value": g.user.speaking},
        {"name": "Writing", "category": "Communication", "icon": "✍️", "value": g.user.writing},
        ...
        {"name": "Organization", "category": "Productivity", "icon": "🗂️", "value": g.user.organization},
        {"name": "Self-Discipline", "category": "Productivity", "icon": "💪", "value": g.user.self_discipline},
        ...
        {"name": "Growth", "category": "Growth", "icon": "🌱", "value": g.user.growth}
    ]
    
    # Combine both lists
    all_skills = built_in_skills + custom_skills_list
    
    # Filter out skills with None values
    all_skills = [skill for skill in all_skills if skill['value'] is not None]
...
def stats():
    ...
    physical_skills = [skill for skill in custom_skills if skill.category == 'Physical']
    mental_skills = [skill for skill in custom_skills if skill.category == 'Mental']
    communication_skills = [skill for skill in custom_skills if skill.category == 'Communication']
    productivity_skills = [skill for skill in custom_skills if skill.category == 'Productivity']
    education_skills = [skill for skill in custom_skills if skill.category == 'Education']
    other_skills = [skill for skill in custom_skills if skill.category == 'Other']
    
    # Calculate average skill values for each category
    skills = {
        'Physical': [
            g.user.strength,
            g.user.endurance,
        ...
        'Communication': [
            ...
            g.user.empathy,
            g.user.persuasion,
            g.user.active_listening
        ],
        'Productivity': [
            g.user.time_management,
            g.user.organization,
            g.user.self_discipline,
            g.user.goal_setting,
            g.user.stress_management
        ],
        'Education': [
            g.user.research,
            g.user.analysis,
            g.user.critical_thinking,
            g.user.problem_solving,
            g.user.strategy
        ],
        'Growth': [
            g.user.growth
    ...
    for category, skill_values in skills.items():
        category_averages[category] = sum(skill_values) / len(skill_values)
...
def complete_objective(id):
    ...
    if g.user.xp >= g.user.xp_to_next_level:
        ...
        g.user.xp_to_next_level = 100 + 15 * g.user.level  # New linear formula
        level_up = True
    ...
    if level_up:
        flash_with_notification(
            f'Congratulations! You have reached Level {g.user.level}!',
            'success',
            'Level Up!'
        )
    ...
    if skill_name.startswith('custom_'):
        try:
            ...
            response = {
                ...
                'skill_name': custom_skill.name  # Add the actual skill name for display in notifications
            }
...
def daily_quests():
    ...
    if settings.auto_submit:
        ...
        if (last_visited_day != day_of_week and 
            ...
            if prev_completed_count > 0 and previous_day not in completed_days and previous_day not in partial_days:
                ...
                completion_multiplier = prev_completed_count / prev_total_count if prev_total_count > 0 else 0
    ...
    # Handle week reset using ISO week number
    if 'current_week' not in session:
        session['current_week'] = current_week
        session['week_start_day'] = day_of_week
        # Initialize completion states for first week
        session['completed_days'] = []
        session['partial_days'] = []
        session['failed_days'] = []
        ...
        session['current_week'] = current_week
        session['week_start_day'] = day_of_week
        # Reset completion states for new week
        session['completed_days'] = []
        session['partial_days'] = []
        session['failed_days'] = []
        session['completed_tasks'] = {}
        # Reset XP penalty if any
        if 'xp_penalty_end' in session:
            del session['xp_penalty_end']
...
def flash_with_notification(message, category='info', title=None):
    ...
    session['browser_notifications'].append({
        ...
        'message': message,
        'type': category,
        'level_up': category == 'success' and title and 'Level Up!' in title  # Special flag for level-up notifications
    })
...
def create_mission():
    ...
    if request.method == 'POST':
        ...
        requirements = request.form['requirements']  # Added requirements field
        ...
        if error is None:
            ...
            mission = Mission(
                title=title,
                description=description,
                requirements=requirements,  # Added requirements field
...
def complete_mission(id):
    ...
    # Get mission with title loaded
    mission = Mission.query.filter_by(id=id, user_id=g.user.id).first()
    if not mission:
        flash_with_notification('Mission not found.', 'error')
        return redirect(url_for('journey.missions'))
    ...
    try:
        ...
        # Reward user with XP
        actual_xp_earned = g.user.add_xp(500)
        ...
        if g.user.level > old_level:
            if penalty_applied:
                penalty_percent = int((1 - multiplier) * 100)
                flash_with_notification(
                    f'Mission completed! You earned {actual_xp_earned} XP ({penalty_percent}% penalty applied) and leveled up to level {g.user.level}!',
                    'success',
                    'Level Up!'
                )
            else:
                flash_with_notification(
                    f'Mission completed! You earned {actual_xp_earned} XP and leveled up to level {g.user.level}!',
                    'success',
                    'Level Up!'
                )
        else:
            if penalty_applied:
                penalty_percent = int((1 - multiplier) * 100)
                flash_with_notification(
                    f'Mission completed! You earned {actual_xp_earned} XP ({penalty_percent}% penalty applied).',
                    'success'
                )
            else:
                flash_with_notification(
                    f'Mission completed! You earned {actual_xp_earned} XP.',
                    'success'
                )
        ...
        db.session.rollback()
        flash_with_notification('An error occurred while completing the mission. Please try again.', 'error')
        return redirect(url_for('journey.missions'))
...
def toggle_topic(topic_id):
    ...
    actual_xp = g.user.add_xp(30)  # Reduced from 50 to 30 XP
...
def add_topic():
    ...
    if topics_added > 0:
        ...
        message = ""
        if topics_skipped > 0:
            message = f'Added {topics_added} new topics successfully! {topics_skipped} topics were skipped (already exist).'
        else:
            message = f'Added {topics_added} new topics successfully!'
...
def add_subtopic():
    ...
    if subtopics_added > 0:
        ...
        if subtopics_skipped > 0:
            message = f'Added {subtopics_added} new subtopics successfully! {subtopics_skipped} subtopics were skipped (already exist).'
        else:
            message = f'Added {subtopics_added} new subtopics successfully!'
...
def toggle_subtopic(subtopic_id):
    ...
    if not was_completed and subtopic.is_completed:
        actual_xp = g.user.add_xp(16)  # Increased from 10 to 16 XP
    ...
    if all_completed and not topic.is_completed:
        ...
        additional_xp = g.user.add_xp(30)  # Reduced from 50 to 30 XP
...
def add_skill():
    ...
    try:
        # Check if this is a built-in skill or a custom skill
        snake_case_name = name.lower().replace(' ', '_')
        
        # Check if the attribute exists on the User model
        if hasattr(User, snake_case_name):
            # This is a built-in skill, check if it's already set
            current_value = getattr(g.user, snake_case_name)
            if current_value is not None:
                return jsonify({'success': False, 'message': f'Skill "{name}" already exists'}), 400
            
            # Set the built-in skill to 1.0 (initial value)
            setattr(g.user, snake_case_name, 1.0)
            db.session.commit()
            ...
            # This is a custom skill, add it to the CustomSkill table
            new_skill = CustomSkill(
                user_id=g.user.id,
                category=category,  # Use the selected category
                category_name=None,  # No custom category name
                category_color=None,  # No custom category color
                name=name,
                icon=icon or '✨',  # Default icon if none provided
                description=description,
        ...
        try:
            ...
            cursor.execute(
                ...
                (g.user.id, category, None, None, name, icon or '✨', description, 1.0)
            )
            ...
            return jsonify({
                'success': True,
                'message': f'Skill "{name}" added successfully (SQLite fallback)',
                'skill_id': skill_id
            })
...
@bp.route('/api/journal_entries', methods=['GET'])
@login_required
def get_journal_entries():
    """Get all journal entries for the current user"""
    # Query database for all journal entries
    entries = db.session.query(JournalEntry).filter_by(user_id=g.user.id).all()
    print(f"Found {len(entries)} journal entries for user {g.user.id}")
    
    # Format for calendar consumption
    formatted_entries = []
    for entry in entries:
        # Use entry_date as the primary date source since most entries use that field
        # Only use date field if it's not the default "2025-01-01" value
        display_date = entry.entry_date
        if entry.date and entry.date.year != 2025:
            display_date = entry.date
            
        print(f"Entry {entry.id} - Date: {entry.date}, Entry_date: {entry.entry_date}, Display: {display_date}")
        
        formatted_entries.append({
            'id': entry.id,
            'title': entry.title or "Untitled Entry",
            'start': display_date.strftime('%Y-%m-%d'),
            'className': f'mood-{entry.mood}' if entry.mood else 'mood-neutral',
            'extendedProps': {
                'mood': entry.mood or 'neutral',
                'content': entry.content,
                'goals': json.loads(entry.goals) if entry.goals else []
            }
        })
...
@bp.route('/api/journal_entries', methods=['POST'])
@login_required
def save_journal_entry():
    ...
    # Parse the date from request regardless of if it's an update or new entry
    entry_date = datetime.strptime(data.get('date'), '%Y-%m-%d').date()
    
    # If entry_id is provided and not a timestamp (client-side temp ID)
    if entry_id and not str(entry_id).isdigit() and len(str(entry_id)) < 13:
        # Update existing entry
        entry = db.session.query(JournalEntry).filter_by(
            id=entry_id, 
            user_id=g.user.id
        ).first()
        ...
        # Always update both date fields to ensure consistency
        ...
        entry = JournalEntry(
            user_id=g.user.id,
            date=entry_date,
            entry_date=entry_date  # Set both date and entry_date fields
        )
    ...
    entry.goals = json.dumps(data.get('goals', []))
    
    # Save to database
    if not entry_id or (str(entry_id).isdigit() and len(str(entry_id)) >= 13):
...
@bp.route('/api/journal_entries/<entry_id>', methods=['DELETE'])
@login_required
...
def delete_skill():
    ...
    try:
        # Check if it's a custom skill (custom skills have format 'custom_ID')
        if skill_name.startswith('custom_'):
            # Extract the ID from the custom skill name
            skill_id = skill_name.split('_')[1]
            custom_skill = CustomSkill.query.filter_by(id=skill_id, user_id=g.user.id).first()
            
            if not custom_skill:
                return jsonify({'success': False, 'message': 'Custom skill not found'}), 404
            
            # Delete the custom skill
            db.session.delete(custom_skill)
            db.session.commit()
            
            flash_with_notification(f'Skill "{custom_skill.name}" has been deleted', 'success')
            return jsonify({'success': True, 'message': 'Custom skill deleted successfully'})
        else:
            # For built-in skills, we'll use SQLAlchemy to set the column to NULL
            # This effectively removes the skill from the user profile
            if hasattr(g.user, skill_name):
                # Set the attribute to None to remove it from the database
                setattr(g.user, skill_name, None)
                db.session.commit()
                
                # Capitalize and format the skill name for display
                display_name = skill_name.replace('_', ' ').title()
                flash_with_notification(f'Skill "{display_name}" has been permanently deleted', 'success')
                return jsonify({'success': True, 'message': 'Skill permanently deleted'})
            else:
                return jsonify({'success': False, 'message': 'Skill not found on user profile'}), 404
    
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'message': f'Error deleting skill: {str(e)}'}), 500

@bp.route('/api/reset_week', methods=['POST'])
@login_required
def reset_week():
    """Reset all weekly quest data to start fresh"""
    try:
        # Reset database records for this user
        QuestCompletionRecord.query.filter_by(user_id=g.user.id).delete()
        db.session.commit()
        
        # Reset all session data related to quests
        session['completed_days'] = []
        session['partial_days'] = []
        session['failed_days'] = []
        session['completed_tasks'] = {}
        ...
        # Reset progress
        session['week_start_day'] = datetime.now().weekday()
        session['current_week'] = datetime.now().isocalendar()[1]
        
        return jsonify({
            'success': True,
            'message': 'Weekly quests have been reset successfully'
        })
    except Exception as e:
        db.session.rollback()
        return jsonify({
            'success': False,
            'message': f'Error resetting week: {str(e)}'
        }), 500